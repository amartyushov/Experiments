...
@UsingSteps(instances = { IntegerSummSteps.class})
public class JBehaveTest extends InjectableEmbedder {
    @Test
    public void run() {
        List<String> storyPaths = new StoryFinder().findPaths(codeLocationFromClass(this.getClass()).getFile(), Arrays.asList("**/*.story"), null);
    ...
    public static class ReportBuilder extends StoryReporterBuilder {
        public ReportBuilder() {
            this.withFormats(Format.CONSOLE, Format.HTML, Format.STATS).withDefaultFormats();
        }
    }
}